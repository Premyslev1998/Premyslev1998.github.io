...
//------------------------------------------------------------------------------------------
//В следующей ф-ии, описан цикл, который будет вып. до тех пор, пока
//мы не дойдем до конца файла - program.asm, и не сгенерируем для каждой
//из команд в данном файле - ОП-коды. 
...
//Замечание!
//В качестве альтернативы конкретной ф-ии можно описать язык-описания
//языка, который бы предварительно загружался и выполнялся.  
void Analyzer::process()
{
	for (auto i=listing_.begin();i<listing_.end();i++)
	{
        Args args;
        split((*i),args);
        curLine_++; //Переменная curLine, содержит
        //порядковый номер текущего байт-кода.
        //Пример:
        //1 2 3 4 5
        //8 3 1   
        //В данном примере след. байткод (curLine) будет равен 4,
        //т.е. curLine=4.
