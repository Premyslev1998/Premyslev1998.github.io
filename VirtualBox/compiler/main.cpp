...
//Конкретная ф-ия выполняет исполнительный файл - program.ve, после того,
//как он был загружен в О/П (массив memory).
//Замечание!
//Данная ф-ия работает с прерываниями.
void interrupt(short intId,unsigned char *memory,short *regs, float *regsFloat)
{
  //cout << "INT " << intId << "\n";
  //Замечание!
  //Структура конкретной ф-ии описано с пом. конструкции switch case,
  //поскольку с помощью ее очень удобно обрабатывать байт-коды.
  //Пример (1):
  //case INT_OUTPUT_INT:  //Если байт-код - 13 (см. файл - defines.h), то выводим элемент массива.
  //cout << regs[0];
  //Пример (2):
  //Если байт-код - прерывание, то вызываем ф-ию обработчик прерывания и т.д.
  switch (intId)
  {
    case INT_EXIT:
    exit(0);
...
//Следующая ф-ия производит проверку... 
//Иными словами она производит сопоставление по ключам (из словаря)
//наименования команд с их байт-кодом и в случае соответствия
//выполняет (эмулирует) определенные действия.
//Замечание!
//Данная ф-ия работает с командами. 
