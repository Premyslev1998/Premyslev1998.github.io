$(function() {
//Замечание!
//При перезагрузки страницы в браузере (на хостинге) js/php-скрипты могут не презагружаться
//поскольку в данном проекте мы используем файл .htaccess, где производится кэширование практически
//всего (для быстрой загрузки). 
//Чтобы избежать этого на странице в браузере (на домене хостинга) сверху в поисковой строке
//(после домена) нужно прописать путь до js-файла проекта (посмотреть пример можно в видео-курсе
//данной верстки ~59:45) - после чего напрямую через (сочетание клавиш) "Ctrl+F5" перезагружем.з 	
